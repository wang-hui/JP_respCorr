...
std::vector<TH1F*> SimhitCorrData::ClosureTestDraw() {
  ...
  TH1F* CaloJetE_MAHIRatio_HB_h = new TH1F("CaloJetE_MAHIRatio_HB_h", "CaloJetE_MAHIRatio_HB_h", 80, 0.0, 4.0);
  TH1F* CaloJetE_MAHIRatio_ieta1516_h = new TH1F("CaloJetE_MAHIRatio_ieta1516_h", "CaloJetE_MAHIRatio_ieta1516_h", 80, 0.0, 4.0);
  TH1F* CaloJetE_MAHIRatio_HE_h = new TH1F("CaloJetE_MAHIRatio_HE_h", "CaloJetE_MAHIRatio_HE_h", 80, 0.0, 4.0);
  TH1F* CaloJetE_MAHIRatio_HE_ietaL_h = new TH1F("CaloJetE_MAHIRatio_HE_ietaL_h", "CaloJetE_MAHIRatio_HE_ietaL_h", 80, 0.0, 4.0);
  TH1F* CaloJetE_MAHIRatio_HE_ietaH_h = new TH1F("CaloJetE_MAHIRatio_HE_ietaH_h", "CaloJetE_MAHIRatio_HE_ietaH_h", 80, 0.0, 4.0);
  ...
  TH1F* CaloJetE_MAHIDiff_HB_h = new TH1F("CaloJetE_MAHIDiff_HB_h", "CaloJetE_MAHIDiff_HB_h", 100, -50, 50);
  TH1F* CaloJetE_MAHIDiff_ieta1516_h = new TH1F("CaloJetE_MAHIDiff_ieta1516_h", "CaloJetE_MAHIDiff_ieta1516_h", 100, -50, 50);
  TH1F* CaloJetE_MAHIDiff_HE_h = new TH1F("CaloJetE_MAHIDiff_HE_h", "CaloJetE_MAHIDiff_HE_h", 100, -50, 50);
  TH1F* CaloJetE_MAHIDiff_HE_ietaL_h = new TH1F("CaloJetE_MAHIDiff_HE_ietaL_h", "CaloJetE_MAHIDiff_HE_ietaL_h", 100, -50, 50);
  TH1F* CaloJetE_MAHIDiff_HE_ietaH_h = new TH1F("CaloJetE_MAHIDiff_HE_ietaH_h", "CaloJetE_MAHIDiff_HE_ietaH_h", 100, -50, 50);

  for(auto& Datum : fData) {
    auto GenEnergy = Datum.getGenJetEnergy();
    auto GenEta = fabs(Datum.getGenJetEta());

    auto OldRatio = (Datum.getSumHcalEDefault() + Datum.getOtherE()) / GenEnergy;
    auto NewRatio = (Datum.getSumHcalE(fRespCorrs) + Datum.getOtherE()) / GenEnergy;
    auto MAHIRatio = Datum.getCaloJetEnergy() / GenEnergy;

    auto OldDiff = (Datum.getSumHcalEDefault() + Datum.getOtherE()) - GenEnergy;
    auto NewDiff = (Datum.getSumHcalE(fRespCorrs) + Datum.getOtherE()) - GenEnergy;
    auto MAHIDiff = Datum.getCaloJetEnergy() - GenEnergy;

    if(GenEta < 1.2) {
        CaloJetE_OldRatio_HB_h->Fill(OldRatio);
        CaloJetE_NewRatio_HB_h->Fill(NewRatio);
        CaloJetE_MAHIRatio_HB_h->Fill(MAHIRatio);

        CaloJetE_OldDiff_HB_h->Fill(OldDiff);
        CaloJetE_NewDiff_HB_h->Fill(NewDiff);
        CaloJetE_MAHIDiff_HB_h->Fill(MAHIDiff);
    }
    else if (GenEta < 1.4) {
        CaloJetE_OldRatio_ieta1516_h->Fill(OldRatio);
        CaloJetE_NewRatio_ieta1516_h->Fill(NewRatio);
        CaloJetE_MAHIRatio_ieta1516_h->Fill(MAHIRatio);

        CaloJetE_OldDiff_ieta1516_h->Fill(OldDiff);
        CaloJetE_NewDiff_ieta1516_h->Fill(NewDiff);
        CaloJetE_MAHIDiff_ieta1516_h->Fill(MAHIDiff);
    }
    else {
        CaloJetE_OldRatio_HE_h->Fill(OldRatio);
        CaloJetE_NewRatio_HE_h->Fill(NewRatio);
        CaloJetE_MAHIRatio_HE_h->Fill(MAHIRatio);

        CaloJetE_OldDiff_HE_h->Fill(OldDiff);
        CaloJetE_NewDiff_HE_h->Fill(NewDiff);
        CaloJetE_MAHIDiff_HE_h->Fill(MAHIDiff);
        if (GenEta < 2.3) {
            CaloJetE_OldRatio_HE_ietaL_h->Fill(OldRatio);
            CaloJetE_NewRatio_HE_ietaL_h->Fill(NewRatio);
            CaloJetE_MAHIRatio_HE_ietaL_h->Fill(MAHIRatio);

            CaloJetE_OldDiff_HE_ietaL_h->Fill(OldDiff);
            CaloJetE_NewDiff_HE_ietaL_h->Fill(NewDiff);
            CaloJetE_MAHIDiff_HE_ietaL_h->Fill(MAHIDiff);
        }
        else {
            CaloJetE_OldRatio_HE_ietaH_h->Fill(OldRatio);
            CaloJetE_NewRatio_HE_ietaH_h->Fill(NewRatio);
            CaloJetE_MAHIRatio_HE_ietaH_h->Fill(MAHIRatio);

            CaloJetE_OldDiff_HE_ietaH_h->Fill(OldDiff);
            CaloJetE_NewDiff_HE_ietaH_h->Fill(NewDiff);
            CaloJetE_MAHIDiff_HE_ietaH_h->Fill(MAHIDiff);
        }
    }
  }
  ...
  HistVec.push_back(CaloJetE_MAHIRatio_HB_h);
  HistVec.push_back(CaloJetE_MAHIRatio_ieta1516_h);
  HistVec.push_back(CaloJetE_MAHIRatio_HE_h);
  HistVec.push_back(CaloJetE_MAHIRatio_HE_ietaL_h);
  HistVec.push_back(CaloJetE_MAHIRatio_HE_ietaH_h);
  ...
  HistVec.push_back(CaloJetE_MAHIDiff_HB_h);
  HistVec.push_back(CaloJetE_MAHIDiff_ieta1516_h);
  HistVec.push_back(CaloJetE_MAHIDiff_HE_h);
  HistVec.push_back(CaloJetE_MAHIDiff_HE_ietaL_h);
  HistVec.push_back(CaloJetE_MAHIDiff_HE_ietaH_h);
